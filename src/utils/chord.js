 ...
 * CONTRACT
 * --------
 * @param {string} symbol   Chord symbol, e.g. "Cmaj7", "G7", "Dm7b5", "Bb13"
 * @param {object} [options]
 *   @param {number} [options.octave=4]  Root octave (tones stay close above root).
 *
 * @returns {string[]}  Scientific-pitch strings, root first.
 ...
 */
function toSPN(rootSemi, octave, intervals) {
  const result = [];
 ...
 */
const QUALITY_TOKENS = Object.keys(QUALITIES).sort((a, b) => b.length - a.length);
 ...
 * @returns {string[]}
 */
export function chordToneArray(symbol, options = {}) {
