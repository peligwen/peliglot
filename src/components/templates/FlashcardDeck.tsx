...
import type { ReactNode, ReactElement, TouchEvent } from 'react';
...
export interface FlashcardDeckProps<T extends { front: ReactNode; back: ReactNode }> {
  items: T[];
  color?: string;
  title?: string;
...
export function FlashcardDeck<T extends { front: ReactNode; back: ReactNode }>({
  items,
  color = '#1565C0',
  title,
  speakFn,
  speakKey = 'front' as keyof T,
}: FlashcardDeckProps<T>): ReactElement {
  const [deck, setDeck] = useState<T[]>(() => shuffle(items));
  ...
  const flip = useCallback(() => {
    if (!flipped) {
      setFlipped(true);
      if (speakFn && current) {
        // speakKey value may be ReactNode; coerce to string for speech
        speakFn(String(current[speakKey] ?? current.front));
      }
    }
  ...
  const onTouchStart = (e: TouchEvent<HTMLDivElement>) => setStartX(e.touches[0].clientX);
  const onTouchEnd = (e: TouchEvent<HTMLDivElement>) => {
    if (startX === null || !flipped) return;
    const dx = e.changedTouches[0].clientX - startX;
    if (Math.abs(dx) > 60) { if (dx > 0) { gotIt(); } else { again(); } }
    setStartX(null);
  };
