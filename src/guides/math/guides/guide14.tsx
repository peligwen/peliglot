...
export function Guide14(){
  ...
  return(<div>
    ...
    <div style={{background:"#fff",borderRadius:14,padding:"16px",border:"1px solid #e0dcd5",marginBottom:16}}>
      ...
      <div style={{display:"flex",justifyContent:"center",margin:"16px 0",flexDirection:"column",alignItems:"center"}}>
        ...
        {r>12&&<div style={{fontSize:10,color:"#aaa",marginTop:4}}>visual scaled to fit — numbers are exact</div>}
      </div>
      ...
      <div style={{padding:"10px",background:"#FDE8FF",borderRadius:8,textAlign:"center",border:"2px solid #8E24AA"}}>
        <div style={{fontSize:10,color:"#888",marginBottom:2}}>Circumference ÷ Diameter (always…)</div>
        <div style={{fontSize:20,fontWeight:800,color:"#6A1B9A"}}>{((2*Math.PI*r)/(r*2)).toFixed(5)} ≈ π</div>
        <div style={{fontSize:10,color:"#aaa"}}>Try any radius — this ratio never changes</div>
      </div>
    </div>
  </div>);
}
