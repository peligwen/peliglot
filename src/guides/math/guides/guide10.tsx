...
export function Guide10(){
  ...
  const rules=[{name:"Double + 1",fn:(x: number)=>2*x+1,formula:"f(x) = 2x + 1"},{name:"Square",fn:(x: number)=>x*x,formula:"f(x) = x²"},{name:"Half - 3",fn:(x: number)=>x/2-3,formula:"f(x) = x/2 - 3"}];
  const r=rules[rule];
  return(<div>
