...
export function Guide21(){
  const [data,_setData]=useState([30,35,40,42,45,45,55,250]);
  const sorted=[...data].sort((a,b)=>a-b);
  const mean=data.reduce((a,b)=>a+b,0)/data.length;
  const mid=Math.floor(sorted.length/2);
  const median=sorted.length%2?sorted[mid]:(sorted[mid-1]+sorted[mid])/2;
  const freq: Record<number,number>={};data.forEach(d=>{freq[d]=(freq[d]||0)+1;});const modeVal=Object.entries(freq).sort((a,b)=>Number(b[1])-Number(a[1]))[0];
  return(<div>
