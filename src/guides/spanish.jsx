    ...
    <div>
      ...
      {sel&&(<div style={{background:"#fff",borderRadius:14,overflow:"hidden",border:"1px solid #e8e5e0",marginBottom:16,animation:"fadeIn 0.2s ease"}}>
        <div style={{background:"#2C5F2D",padding:"16px 20px",display:"flex",alignItems:"center",gap:16}}>
          ...
          <div><div style={{color:"rgba(255,255,255,0.6)",fontSize:11}}>Name: <strong style={{color:"#fff"}}>{sel.name}</strong></div><div style={{color:"#FFE77A",fontSize:16,fontFamily:"monospace",fontWeight:700}}>{sel.ipa}</div></div>
        </div>
...
function VowelBar(){
  ...
  return(<div style={{background:"#fff",borderRadius:12,overflow:"hidden",border:"1px solid #e8e5e0"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)"}}>
      {vs.map((v,i)=>(<div key={v.l} style={{padding:"12px 6px",textAlign:"center",borderRight:i<4?"1px solid #f0eeeb":"none"}}>
        <div style={{fontSize:24,fontWeight:700,color:"#2C5F2D"}}>{v.l}</div>
        <div style={{fontSize:14,color:"#D4A843",fontWeight:700,fontFamily:"monospace"}}>{v.s}</div>
...
// ═══════════════════════════════════════════════════════════════
// GUIDE 4-10: VERB TENSES (Shared component pattern)
// ═══════════════════════════════════════════════════════════════
const pronouns6=["yo","tú","él/ella/Ud.","nosotros","vosotros","ellos/Uds."];
const pronounsShort=["yo","tú","él","nos.","vos.","ellos"];
...
function ConjugationTable({stem,endings,verb,meaning,color,pronouns=pronouns6}){
...
function Guide6(){
  ...
  return(<div>
    <DarkBox title="The Big Shortcut"><div style={{fontSize:14}}>Both tenses attach endings to the <strong style={{color:"#FFE77A"}}>full infinitive</strong>. No dropping -ar/-er/-ir!</div></DarkBox>
...
function Guide7(){
  ...
  return(<div>
    <DarkBox title="The Formula"><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
    ...
    <Insight text="Don't overuse it! Spanish uses the progressive much less than English. It emphasizes right-this-moment actions."/>
  </div>);
}
...
function Guide8(){
  const [at,setAt]=useState("pp");const t=haberForms[at];
  return(<div>
...
function Guide12(){return(<div>{pluralRules.map((r,i)=>(<Card key={i} color={r.color} title={r.l}>{r.ex.map((e,j)=>(<div key={j} style={{display:"flex",alignItems:"center",padding:"8px 16px",borderBottom:j<r.ex.length-1?"1px solid #f0eeeb":"none"}}><span style={{flex:1,fontSize:15,color:"#999"}}>{e.s}</span><span style={{fontSize:18,color:"#ccc",margin:"0 12px"}}>→</span><span style={{flex:1,fontSize:15,fontWeight:700,color:r.color}}>{e.p}</span></div>))}</Card>))}</div>);}

function Guide13(){return(<SimpleGuide title="Adjective Agreement" items={[
  {h:"4-Form (-o/-a/-os/-as)",b:"Most adjectives: alto/alta/altos/altas"},
  {h:"2-Form (-e / consonant)",b:"Only number changes: grande → grandes, difícil → difíciles"},
  {h:"Nationality adds feminine -a",b:"español/española, francés/francesa, alemán/alemana"},
  {h:"Default position: AFTER noun",b:"la casa roja = the red house (distinguishing)"},
  {h:"Before noun = subjective/emotional",b:"la hermosa vista = the beautiful view"},
  {h:"⚠ Meaning shifts",b:"pobre mujer (unfortunate) vs mujer pobre (no money)\nviejo amigo (long-time) vs amigo viejo (elderly)\ngran hombre (great) vs hombre grande (big)"},
]}/>);}
...
function Guide15(){return(<SimpleGuide title="Pronoun Placement" items={[
  {h:"Before conjugated verb",b:"Lo veo. (I see him.) Me llama. (She calls me.)"},
  {h:"Attached to infinitive",b:"Quiero verlo. = Lo quiero ver. (I want to see it.)"},
  {h:"Attached to gerund",b:"Estoy haciéndolo. = Lo estoy haciendo."},
  {h:"Attached to affirmative command",b:"¡Dímelo! (Tell it to me!) ¡Hazlo! (Do it!)"},
  {h:"Before negative command",b:"¡No lo hagas! (Don't do it!) ¡No me digas!"},
  {h:"Double pronouns: IO before DO",b:"Le + lo → SE lo. ¡Le lo is NEVER correct!"},
  {h:"Order: se → IO → DO",b:"Se lo digo. (I tell it to him.) Mnemonic: RID"},
]}/>);}
...
function Guide19(){return(<SimpleGuide title="Verb + Preposition Pairs" items={[
  {h:"pensar en = think about",b:"Pienso en ti. ≠ pensar de = have opinion of"},
  {h:"soñar con = dream about",b:"Soñé con volar."},
  {h:"salir de = leave from / salir con = date",b:"Sale con María."},
  {h:"enamorarse de = fall in love with",b:"Se enamoró de ella."},
  {h:"depender de = depend on",b:"Depende de ti."},
  {h:"acabar de = to have just done",b:"Acabo de llegar. (I just arrived.)"},
  {h:"⚠ No preposition needed",b:"buscar = to look for · escuchar = to listen to\nesperar = to wait for · pedir = to ask for"},
]}/>);}
...
function Guide22(){return(<SimpleGuide title="Comparatives & Superlatives" items={[
  {h:"más + adj + que = more...than",b:"Ella es más alta que yo. (She's taller than me.)"},
  {h:"menos + adj + que = less...than",b:"Es menos caro que el otro."},
  {h:"tan + adj + como = as...as",b:"Soy tan alto como mi papá."},
  {h:"tanto/a/os/as + noun + como",b:"Tengo tantos amigos como tú. (tanto agrees!)"},
  {h:"Superlative: el/la más + adj + de",b:"Es el más alto de la clase. (Use DE not 'en')"},
  {h:"⚠ Irregulars — don't say 'más bueno'",b:"bueno → mejor · malo → peor\ngrande → mayor (age) · pequeño → menor (age)"},
]}/>);}

function Guide23(){return(<SimpleGuide title="Numbers, Dates & Time" items={[
  {h:"0-15: unique forms to memorize",b:"cero, uno, dos...diez, once, doce, trece, catorce, quince"},
  {h:"16-29: compound words",b:"dieciséis...veinte, veintiuno...veintinueve"},
  {h:"30+: tens + y + unit",b:"treinta y cinco = 35. Cien = 100, mil = 1000"},
  {h:"Date format: el + day + de + month + de + year",b:"el 5 de mayo de 2024. Days & months NOT capitalized."},
  {h:"Time: ¿Qué hora es?",b:"Es la una (1:00 singular). Son las dos/tres... (2+ plural)\ny media = :30 · y cuarto = :15 · menos cuarto = :45"},
  {h:"🇲🇽 12-hour + de la mañana/tarde/noche",b:"Son las tres de la tarde. = 3:00 PM"},
]}/>);}
...
function MiniTable({title,color,stem,endings}){
...
function TriggerChips({label,color,words}){
  return(<div style={{background:"#fff",borderRadius:12,overflow:"hidden",border:"1px solid #eee"}}>
...
// ═══════════════════════════════════════════════════════════════
const guideComponents=[Guide1,Guide2,Guide3,Guide4,Guide5,Guide6,Guide7,Guide8,Guide9,Guide10,Guide11,Guide12,Guide13,Guide14,Guide15,Guide16,Guide17,Guide18,Guide19,Guide20,Guide21,Guide22,Guide23,Guide24,Guide25];

// ═══════════════════════════════════════════════════════════════
// MAIN APP
// ═══════════════════════════════════════════════════════════════
export default function SpanishGuide(){
  ...
  const next=()=>{if(page<24)goTo(page+1);};
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#FAFAF6",fontFamily:"'Segoe UI','Helvetica Neue',sans-serif",overflow:"hidden",position:"relative"}}>
      <style>{`@keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}@keyframes slideIn{from{transform:translateX(-100%)}to{transform:translateX(0)}}*{box-sizing:border-box}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#ddd;border-radius:4px}`}</style>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #eee",padding:"10px 16px",display:"flex",alignItems:"center",gap:12,flexShrink:0,zIndex:10}}>
        ...
        <div style={{background:meta.color,color:"#fff",padding:"3px 10px",borderRadius:6,fontSize:10,fontWeight:700,flexShrink:0}}>
          {page+1}/25
        </div>
      </div>
      ...
      <div style={{background:"#fff",borderTop:"1px solid #eee",padding:"8px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:3}}>
          {guidesMeta.map((_,i)=>(<div key={i} onClick={()=>goTo(i)} style={{width:i===page?16:6,height:6,borderRadius:3,background:i===page?meta.color:"#ddd",cursor:"pointer",transition:"all 0.2s"}}/>))}
        </div>
        <button onClick={next} disabled={page===24} style={{padding:"8px 20px",borderRadius:10,border:"1.5px solid #e0ddd8",background:page===24?"#f5f5f5":"#fff",color:page===24?"#ccc":"#555",fontSize:13,fontWeight:700,cursor:page===24?"default":"pointer"}}>Next →</button>
      </div>
      ...
      <div style={{position:"fixed",top:0,left:0,bottom:0,width:280,background:"#fff",zIndex:30,transform:menuOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.25s ease",boxShadow:menuOpen?"4px 0 24px rgba(0,0,0,0.15)":"none",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid #eee",flexShrink:0}}>
          ...
          <div style={{fontSize:12,color:"#999"}}>25 Interactive Guides</div>
        </div>
