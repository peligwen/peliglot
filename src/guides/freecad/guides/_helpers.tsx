...
// ── Shared helper components for the FreeCAD guide collection ─────────────
...
export function Ref({ name, text }: { name: string; text: string }): ReactElement {
  return (
    <div style={{ background: "#0f1e2d", borderRadius: 10, padding: "10px 14px", marginBottom: 12, border: "1px solid #243a52", fontSize: 12, color: "#8fa3b8", lineHeight: 1.5 }}>
...
type GeoLine        = { type: 'line';   x1: number; y1: number; x2: number; y2: number; construction?: boolean };
type GeoCircle      = { type: 'circle'; cx: number; cy: number; r: number;              construction?: boolean };
type GeoArc         = { type: 'arc';    cx: number; cy: number; r: number; start: number; end: number };
type GeoPoint       = { type: 'point';  x: number;  y: number };
export type GeoShape = GeoLine | GeoCircle | GeoArc | GeoPoint;

type ConstraintCoincident    = { type: 'coincident';    at: Point };
type ConstraintHorizontal    = { type: 'horizontal';    at: Point };
type ConstraintVertical      = { type: 'vertical';      at: Point };
type ConstraintParallel      = { type: 'parallel';      at: Point };
type ConstraintPerpendicular = { type: 'perpendicular'; at: Point };
type ConstraintTangent       = { type: 'tangent';       at: Point };
type ConstraintEqual         = { type: 'equal';         at: Point };
type ConstraintSymmetric     = { type: 'symmetric';     at: Point };
type ConstraintFix           = { type: 'fix';           at: Point };
export type SketchConstraint =
  | ConstraintCoincident | ConstraintHorizontal | ConstraintVertical
  | ConstraintParallel | ConstraintPerpendicular | ConstraintTangent
  | ConstraintEqual | ConstraintSymmetric | ConstraintFix;

export type SketchDimension = { from: Point; to: Point; value: string };
...
export function SketchDiagram({
  ...
  height = 220,
  bg = "#0f1e2d",
}: SketchDiagramProps): ReactElement {
  ...
  for (let x = 0; x <= width; x += GRID_MINOR) {
    gridLines.push(<line key={"gx" + x} x1={x} y1={0} x2={x} y2={height} stroke={x % GRID_MAJOR === 0 ? "#1e3a56" : "#152333"} strokeWidth={x % GRID_MAJOR === 0 ? 0.8 : 0.4} />);
  }
  for (let y = 0; y <= height; y += GRID_MINOR) {
    gridLines.push(<line key={"gy" + y} x1={0} y1={y} x2={width} y2={y} stroke={y % GRID_MAJOR === 0 ? "#1e3a56" : "#152333"} strokeWidth={y % GRID_MAJOR === 0 ? 0.8 : 0.4} />);
  }

  function renderGeo(g: GeoShape, i: number): ReactElement | null {
    const hl = highlights.includes(i); const col = hl ? accentColor : normalColor; const sw = hl ? 2 : 1;
    if (g.type === "line") {
      const dash = g.construction ? "6,4" : undefined;
      return <line key={i} x1={g.x1} y1={g.y1} x2={g.x2} y2={g.y2} stroke={col} strokeWidth={sw} strokeDasharray={dash} />;
    }
    if (g.type === "circle") {
      const dash = g.construction ? "6,4" : undefined;
      return <circle key={i} cx={g.cx} cy={g.cy} r={g.r} fill="none" stroke={col} strokeWidth={sw} strokeDasharray={dash} />;
    }
    if (g.type === "arc") {
    ...
    if (g.type === "point") {
      return <circle key={i} cx={g.x} cy={g.y} r={3} fill={col} />;
    }
    return null;
  }

  function renderConstraint(c: SketchConstraint, i: number): ReactElement | null {
  ...
  function renderDimension(d: SketchDimension, i: number): ReactElement {
...
export function FeatureTree({ nodes = [], title }: FeatureTreeProps): ReactElement {
  function renderNode(node: FeatureTreeNode, depth: number): ReactElement {
  ...
  return (
    <div style={{ background: "#162a3d", borderRadius: 12, border: "1px solid #243a52", padding: "10px 12px", marginBottom: 16 }}>
