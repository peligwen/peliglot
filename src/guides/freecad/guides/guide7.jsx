...
export function Guide7(){
  ...
  function polyGeo(n,cx,cy,r){
    const pts=Array.from({length:n},(_,i)=>{const a=(2*Math.PI*i/n)-(Math.PI/2);return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};});
    return pts.map((p,i)=>({type:"line",x1:p.x,y1:p.y,x2:pts[(i+1)%n].x,y2:pts[(i+1)%n].y}));
  }
  const cornerRectGeo=[{type:"line",x1:80,y1:60,x2:240,y2:60},{type:"line",x1:240,y1:60,x2:240,y2:150},{type:"line",x1:240,y1:150,x2:80,y2:150},{type:"line",x1:80,y1:150,x2:80,y2:60}];
