...
export function Guide6(){
  ...
  const modes=[
    ...
    {label:"Tangent Arc",shortcut:"G, A → M",desc:"Draw an arc with G, A, then press M to cycle modes — the endpoint-tangent mode auto-applies the tangent constraint. In FreeCAD 1.0 the unified Arc tool will auto-tangent at an existing endpoint."},
  ];
  const m=modes[mode];
  return(<div>
    <DarkBox title="CIRCLES & ARCS"><div style={{fontSize:14,lineHeight:1.6,color:"#e8ecf0"}}>
      Three modes for circles, three for arcs. Pick the mode that matches what you already know. <strong style={{color:"#f39c12"}}>Center+radius</strong> is most common. <strong style={{color:"#f39c12"}}>3-point</strong> is best when you know where the curve passes. <strong style={{color:"#f39c12"}}>Tangent arc</strong> builds sketch fillets automatically — draw with G, A and press M to select the tangent-at-endpoint mode.
    </div></DarkBox>
    ...
    {mode===2&&<SketchDiagram
      title="Tangent Arc (G, A → M) — arc from line endpoint, tangent built in"
      width={320} height={200}
      geometry={[
    ...
    <Insight text="Tangent arcs from a line endpoint are how you build fillets into sketch profiles — constraints already wired up, no manual tangent needed. Use G, A to start the arc tool and press M to cycle to the endpoint-tangent mode." />
  </div>);
}
