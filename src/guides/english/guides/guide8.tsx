...
export function Guide8(){
  ...
  const data={will:{label:"WILL",desc:"Espontáneo / predicción / promesa",ex:["I'll help you. (decisión espontánea)","It will rain tomorrow. (predicción)","I'll call you back. (promesa)"],color:"#1565C0"},going:{label:"GOING TO",desc:"Planificado / basado en evidencia",ex:["I'm going to visit my mom. (ya planificado)","Look — it's going to rain. (evidencia visible)"],color:"#2E7D32"},prog:{label:"PRESENTE PROGRESIVO",desc:"Agendado / con boleto",ex:["I'm flying to Atlanta on Friday. (ya tiene boleto)","We're meeting at 3. (ya confirmado)"],color:"#6A1B9A"},fixin:{label:"FIXIN' TO",desc:"Sur de EEUU: a punto de / estoy por",ex:["I'm fixin' to go to the store. (salgo en un momento)","She's fixin' to leave. (está a punto de irse)","Finna do it. ('finna' = fixin' to, más urbano/joven)"],color:"#E65100"}};
  const d=data[mode as keyof typeof data];
  return(<div>
    <div style={{display:"flex",gap:6,marginBottom:14,justifyContent:"center"}}>
      {Object.entries(data).map(([k,v])=>(<button key={k} onClick={()=>setMode(k)} style={{flex:1,maxWidth:130,padding:"8px 4px",borderRadius:10,border:mode===k?`2.5px solid ${v.color}`:"1.5px solid #ddd",background:mode===k?v.color:"#fff",color:mode===k?"#fff":"#666",cursor:"pointer",fontWeight:700,fontSize:11,textAlign:"center"}}>{v.label}</button>))}
    </div>
