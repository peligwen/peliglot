    ...
    <>
      ...
      <Card color="#1565C0" title="Key Concepts" subtitle="Absolute vs. relative">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          <div style={{ padding: 14, borderRadius: 10, background: "#0D1F33", border: "1px solid #1A3A5C" }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#42A5F5" }}>dBm</div>
            <div style={{ fontSize: 12, color: "#B0BEC5", lineHeight: 1.6, marginTop: 6 }}>
            ...
            <thead>
              <tr>
                {["Component", "Typical Loss"].map(h => (
                  <th key={h} style={{ padding: "8px 12px", textAlign: "left", color: "#8BACC8", borderBottom: "2px solid #1A3A5C", fontWeight: 700 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {lossSources.map(s => (
                <tr key={s.label}>
                  <td style={{ padding: "7px 12px", borderBottom: "1px solid #1A3A5C", color: "#E0F7FA" }}>{s.icon} {s.label}</td>
                  <td style={{ padding: "7px 12px", borderBottom: "1px solid #1A3A5C", color: "#FFB74D", fontWeight: 700 }}>{s.loss} dB</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card color="#2E7D32" title="Loss Budget Calculator" subtitle="Enter your link values">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          {[
            ...
            <div key={f.label}>
              <div style={{ fontSize: 11, color: "#8BACC8", marginBottom: 4, fontWeight: 600 }}>{f.label}</div>
              <input type="number" value={f.value} min={f.min} max={f.max} step={f.step}
                onChange={e => f.set(parseFloat(e.target.value) || 0)}
                style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid #1A3A5C", background: "#0D1F33", color: "#E0F7FA", fontSize: 14, fontWeight: 700 }}
              />
            </div>
          ))}
          <div style={{ gridColumn: "1 / -1" }}>
            <div style={{ fontSize: 11, color: "#8BACC8", marginBottom: 4, fontWeight: 600 }}>Splitter Ratio</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {Object.keys(splitterLoss).map(r => (
                <button key={r} onClick={() => setRatio(r)} style={{
                  ...
                  background: ratio === r ? "#2E7D32" : "#132D4A",
                  color: ratio === r ? "#fff" : "#8BACC8",
                  border: `1px solid ${ratio === r ? "#4CAF50" : "#1A3A5C"}`,
                }}>{r}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: "#8BACC8", marginBottom: 4 }}>Total Loss: {totalLoss.toFixed(1)} dB</div>
