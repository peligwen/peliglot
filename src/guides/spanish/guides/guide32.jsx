...
export function Guide32(){
  const [cat,setCat]=useState("tener");
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:14}}>
      {Object.entries(groups).map(([k,v])=>(<button key={k} onClick={()=>{setCat(k);setPro(-1);}} style={{padding:"7px 16px",borderRadius:8,border:cat===k?"2px solid #1a1a1a":"1.5px solid #ddd",background:cat===k?v.color:"#fff",color:cat===k?"#fff":"#666",fontSize:12,fontWeight:700,cursor:"pointer"}}>{v.label}</button>))}
    </div>
