...
export function Guide8(){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5,marginBottom:5}}>
      {(Object.entries(haberForms) as [PerfectKey, typeof haberForms[PerfectKey]][]).slice(0,3).map(([k,v])=>(<button key={k} onClick={()=>setAt(k)} style={{padding:"8px 4px",borderRadius:8,border:at===k?`2.5px solid ${v.c}`:"1.5px solid #ddd",background:at===k?v.c:"#fff",color:at===k?"#fff":"#555",cursor:"pointer",fontSize:10,fontWeight:700,textAlign:"center",lineHeight:1.2}}>{v.l}<div style={{fontSize:8,marginTop:2,opacity:0.8}}>{v.freq}</div></button>))}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5,marginBottom:16}}>
      {(Object.entries(haberForms) as [PerfectKey, typeof haberForms[PerfectKey]][]).slice(3).map(([k,v])=>(<button key={k} onClick={()=>setAt(k)} style={{padding:"8px 4px",borderRadius:8,border:at===k?`2.5px solid ${v.c}`:"1.5px solid #ddd",background:at===k?v.c:"#fff",color:at===k?"#fff":"#555",cursor:"pointer",fontSize:10,fontWeight:700,textAlign:"center",lineHeight:1.2}}>{v.l}<div style={{fontSize:8,marginTop:2,opacity:0.8}}>{v.freq}</div></button>))}
    </div>
