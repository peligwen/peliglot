...
export function PromptBox({ prompt, response, label, color = "#6A1B9A" }: PromptBoxProps): ReactElement {
  return (
...
export function Tip({ text }: { text: string }): ReactElement {
  return <BaseInsight text={text} emoji="🤖" bg="#E3F2FD" border="#90CAF9" color="#1565C0" />;
}

/* Term — inline highlighted AI term */
export function Term({ children, color = "#6A1B9A" }: { children: ReactNode; color?: string }): ReactElement {
