...
export function Guide8(){
  ...
  return(<div>
    ...
    <Piano keys={13} highlighted={chord} highlightColor={types.find(t=>t.k===type)?.c ?? "#1565C0"} labels={Object.fromEntries(chord.map((n,i)=>[n,lbls[i]||""]))} />
    <div style={{background:"#fff",borderRadius:12,padding:"10px 14px",border:"1px solid #e0dcd5",marginBottom:12}}>
      <div style={{fontSize:15,fontWeight:700,color:types.find(t=>t.k===type)?.c ?? "#1565C0"}}>{root}{type==="maj7"?"maj7":type==="min7"?"m7":type==="dom7"?"7":type==="dim7"?"dim7":"m7b5"}: {chord.join(" – ")}</div>
