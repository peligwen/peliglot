import { SimpleGuide } from '../../../components/SimpleGuide';
import { CultureNote } from './_helpers';

export function Guide27(){return(<div>
  <SimpleGuide items={[
    {h:"Land & water",b:"\u02BB\u0101ina = land \u00B7 honua = earth \u00B7 moana = deep ocean\nkai = sea/saltwater \u00B7 wai = freshwater \u00B7 kahawai = stream/river\nmauna = mountain \u00B7 kuahiwi = mountain range \u00B7 pali = cliff"},
    {h:"Sky & weather",b:"lani = sky/heaven \u00B7 l\u0101 = sun \u00B7 mahina = moon\nao = cloud \u00B7 ua = rain \u00B7 makani = wind\n\u0101nuenue = rainbow \u00B7 h\u014Dk\u016B = star"},
    {h:"Plants & animals",b:"l\u0101\u02BBau = tree/plant \u00B7 pua = flower \u00B7 niu = coconut\nkalo = taro (sacred staple) \u00B7 \u02BBulu = breadfruit\ni\u02BBa = fish \u00B7 manu = bird \u00B7 honu = sea turtle\nmano = shark \u00B7 nai\u02BBa = dolphin \u00B7 kohol\u0101 = whale"},
    {h:"Place name meanings",b:"Waik\u012Bk\u012B = spouting water \u00B7 Honolulu = sheltered bay\nMauna Kea = white mountain \u00B7 Haleak\u0101l\u0101 = house of the sun\nKalan\u012B\u02BB\u0101na\u02BBole = the chief beyond measure \u00B7 \u02BB\u012Bao = cloud supreme\nHawaiian place names describe the land \u2014 learning them teaches geography AND vocabulary"},
    {h:"Place name pronunciation guide",b:"Apply the CV rule and stress pattern to every Hawaiian place name:\nWai-k\u012B-k\u012B: stress on k\u012B (long vowel, 2nd-to-last mora)\nHo-no-lu-lu: stress on lu (penultimate mora)\nHa-le-a-ka-l\u0101: stress on the final long \u0101 \u00B7 Mau-na Ke-a: mau-NA KEH-ah\nTip: sound out every letter, never skip the \u02BBokina \u2014 it changes both meaning and stress."},
    {h:"\u02BB\u014Clelo No\u02BBeau (Hawaiian proverbs)",b:"Hawaiian proverbs (\u02BB\u014Clelo No\u02BBeau) encode wisdom through nature imagery (Pukui, \u02BB\u014Clelo No\u02BBeau, 1983):\n\u00B7 \"I ka \u02BB\u014Blelo n\u014B ke ola, i ka \u02BB\u014Blelo n\u014B ka make.\"\n   In language there is life, in language there is death. (Language preserves or destroys.)\n\u00B7 \"Ma ka hana ka \u02BBike.\"\n   In doing, there is knowing. (Learning through practice.)\n\u00B7 \"\u02BB\u0100ina ho\u02BBopulapula.\"\n   Land that is planted flourishes. (Cultivate what you have.)"},
  ]}/>
  <CultureNote text="In Hawaiian worldview, people belong TO the land (\u02BB\u0101ina), not the other way around. This is reflected in the O-class possessive: ko\u02BBu \u02BB\u0101ina = my land (the land that holds me, not land I own)."/>
</div>);}
