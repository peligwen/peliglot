...
const categories=["Sounds","Sentences","Nouns","Pronouns","Verbs","Vocabulary","Culture"];
const catColors={Sounds:"#1B5E20",Sentences:"#0D47A1",Nouns:"#C62828",Pronouns:"#1B5E20",Verbs:"#C62828",Vocabulary:"#4527A0",Culture:"#880E4F"};
...
function DarkBox({title,children}){
  return(<div style={{background:"linear-gradient(135deg,#1a1a1a,#2a2520)",borderRadius:14,padding:"16px 20px",marginBottom:16,color:"#fff",textAlign:"center"}}>
...
function CultureNote({text}){return(<div style={{background:"#E8F5E9",borderRadius:10,padding:"10px 14px",marginBottom:12,border:"1px solid #C8E6C9",fontSize:12,color:"#2E7D32",lineHeight:1.5}}>🌿 <strong>Cultural note:</strong> {text}</div>);}
...
function SimpleGuide({items}){
  return(<div>{items.map((item,i)=>(<div key={i} style={{background:"#fff",borderRadius:10,padding:"10px 14px",border:"1px solid #e0dcd5",marginBottom:6}}>
...
function ExpandSection({label,color,children}){
  ...
  return(<div style={{marginBottom:8}}>
    <button onClick={()=>setOpen(!open)} style={{width:"100%",padding:"10px 16px",borderRadius:10,border:"1.5px solid #e0dcd5",background:open?color||"#1a1a1a":"#fff",color:open?"#fff":"#555",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
...
// MAIN APP
// ═══════════════════════════════════════════════════════════════
export default function HawaiianGuide(){
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#FDFBF7",fontFamily:"'Georgia','Garamond','Times New Roman',serif",overflow:"hidden",position:"relative"}}>
      <style>{`@keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}*{box-sizing:border-box}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#d4cfc5;border-radius:4px}`}</style>

      {/* TOP BAR */}
      <div style={{background:"#fff",borderBottom:"1px solid #e0dcd5",padding:"10px 16px",display:"flex",alignItems:"center",gap:12,flexShrink:0,zIndex:10}}>
        <button onClick={()=>setMenuOpen(!menuOpen)} style={{width:36,height:36,borderRadius:10,border:"1.5px solid #e0dcd5",background:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,color:"#555",flexShrink:0,fontFamily:"system-ui,sans-serif"}}>☰</button>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:800,color:"#1a1a1a",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"system-ui,sans-serif"}}>{meta.icon} {meta.subtitle}</div>
          <div style={{fontSize:12,color:"#999",fontStyle:"italic"}}>{meta.title}</div>
        </div>
        <div style={{background:meta.color,color:"#fff",padding:"3px 10px",borderRadius:6,fontSize:10,fontWeight:700,flexShrink:0,fontFamily:"system-ui,sans-serif"}}>{page+1}/30</div>
      </div>

      {/* CONTENT */}
      <div ref={contentRef} style={{flex:1,overflow:"auto",padding:"16px",position:"relative"}}>
        <div key={page} style={{animation:"fadeIn 0.2s ease",maxWidth:700,margin:"0 auto",fontFamily:"system-ui,'Segoe UI',sans-serif"}}>
          <GuideComp/>
        </div>
      </div>

      {/* BOTTOM NAV */}
      <div style={{background:"#fff",borderTop:"1px solid #e0dcd5",padding:"8px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,fontFamily:"system-ui,sans-serif"}}>
        <button onClick={prev} disabled={page===0} style={{padding:"8px 20px",borderRadius:10,border:"1.5px solid #e0dcd5",background:page===0?"#f5f5f5":"#fff",color:page===0?"#ccc":"#555",fontSize:13,fontWeight:700,cursor:page===0?"default":"pointer"}}>← Prev</button>
        ...
        <button onClick={next} disabled={page===29} style={{padding:"8px 20px",borderRadius:10,border:"1.5px solid #e0dcd5",background:page===29?"#f5f5f5":"#fff",color:page===29?"#ccc":"#555",fontSize:13,fontWeight:700,cursor:page===29?"default":"pointer"}}>Next →</button>
      </div>

      {/* SIDE MENU */}
      {menuOpen&&<div onClick={()=>setMenuOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:20}}/>}
      <div style={{position:"fixed",top:0,left:0,bottom:0,width:280,background:"#fff",zIndex:30,transform:menuOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.25s ease",boxShadow:menuOpen?"4px 0 24px rgba(0,0,0,0.15)":"none",display:"flex",flexDirection:"column",fontFamily:"system-ui,sans-serif"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid #e0dcd5",flexShrink:0}}>
          <div style={{fontSize:18,fontWeight:800,color:"#1a1a1a"}}>ʻŌlelo Hawaiʻi</div>
              ...
              {items.map(g=>{const idx=g.id-1;const isActive=idx===page;return(
                <button key={g.id} onClick={()=>goTo(idx)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"8px 20px",border:"none",background:isActive?`${g.color}12`:"transparent",cursor:"pointer",textAlign:"left",borderLeft:isActive?`3px solid ${g.color}`:"3px solid transparent"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:10,color:"#999",fontStyle:"italic"}}>{g.title}</div>
                  </div>
