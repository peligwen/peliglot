  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(64px, 1fr))",gap:5,marginBottom:14}}>
      {filtered.map((lt,i)=>{const isSel=sel===i;return(
        <button key={i} onClick={()=>setSel(isSel?null:i)} style={{padding:"10px 4px",border:isSel?`2.5px solid ${lt.color}`:"1.5px solid #e0dcd5",borderRadius:10,background:isSel?lt.color:"#fff",color:isSel?"#fff":"#1a1a1a",cursor:"pointer",textAlign:"center",transition:"all 0.15s",transform:isSel?"scale(1.05)":"scale(1)"}}>
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#FAFAF6",fontFamily:"'Segoe UI','Helvetica Neue',sans-serif",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{position:"fixed",top:0,left:0,bottom:0,width:280,background:"#fff",zIndex:30,transform:menuOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.25s ease",boxShadow:menuOpen?"4px 0 24px rgba(0,0,0,0.15)":"none",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid #eee",flexShrink:0}}>
          <div style={{fontSize:18,fontWeight:800,color:"#1a1a1a"}}>Deutsch</div>
          <div style={{fontSize:12,color:"#999"}}>33 Interactive Guides</div>
        </div>
